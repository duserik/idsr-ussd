...
function process_response( $method_name, $params, $app_data)
{
	...
	else
	{
		...
		if ( $userdetails != NULL ){
			$responseString = "Bonjour ".$userdetails['firstname']." ".$userdetails['Lastname']." Bienvenue à eIDSR rapports";
			$action = "notify";
			$encode = "UCS2-FR";
		}else{
			$responseString = "Desole, vous n'etes pas enregistre pour acceder a cette application";
			$action = "end";
			$encode = "UCS2-FR";
		}
		$response =array("TransactionId"=>$params[0]["TransactionId"],"TransactionTime"=>$params[0]["TransactionTime"],"USSDResponseString"=> $responseString,"action"=>$action,"USSDEncoding"=> $encode);
	}
	$file = "allrequests-phones.txt";
	$date = date("Y m d H:i");
        file_put_contents($file, "$phone $date \r\n", FILE_APPEND);
	return $response;
}
